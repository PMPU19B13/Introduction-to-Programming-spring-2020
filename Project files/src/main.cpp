...
public:
	...
	Segment* getSegmentPointer() const
	{
		return m_segment;
	}
...
{
public:
	Totals(const Storage<verticalErrorType>& storage) : m_verticalErrs(storage)
	{
	}
	...
	{
		double res = 0;
		Storage<verticalErrorType>::Marker marker = m_verticalErrs.createMarker();
...
private:
	Storage<verticalErrorType> m_verticalErrs;
};
*/
...
int main()
{
	try
	{
		...
		//Storage<verticalErrorType> storage;
		//storage.add(vt);
		//storage.add(vt1);
		...
		std::cout << "res = " << res << " vt1(res) = " << vt1(1) << std::endl;
